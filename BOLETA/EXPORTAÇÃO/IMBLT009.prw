...
User Function IMBLT009()

	Local aArea				:= GetArea()
	Local oArea				:= FWLayer():New()
	Local cPerg				:= "IMBLT009"
	...
	AADD(aCols, {.F.,"@BMP",CToD("  /  /  "),"","","",CToD("  /  /  "),"","","","","","","","","","",CToD("  /  /  "),CToD("  /  /  "),"","","","",""})
	...
	oArea:AddWindow("L01FRET" ,"L01FRET"  ,"Registros de Alterações de Pastores/Bispos"	, 100,.F.,.F.,/*bAction*/,"L02",/*bGotFocus*/)		
...
//	@ 	0,   0 LISTBOX oEstoque		FIELDS HEADER "","","Data","Hora","Igreja","Descrição","Endereço","Numero","Bairro","Cod Mun","Municipio","Estado","CEP","Email","Centro de Custo","Igreja Superior","Tipo","Status" FIELDSIZES 15,15,15,15,15,15,30,40,30,35,120,15,30,50,50,40,30,30 SIZE oPainRank:nClientWidth/2,oPainRank:nClientHeight/2 OF oPainRank

	@ 	0,   0 LISTBOX oEstoque		FIELDS HEADER "","","Data","Hora","Matricula","Nome","Nascimento","Sexo","CIC","RG","Est. Civil","Conjuge","Igreja","Salario","Tipo","Banco Salario","Conta Salario","Status","Admissão","Demissão","Hierarquia","Centro de Custo","Tipo" FIELDSIZES 15,15,15,15,20,15,20,15,30,35,20,45,30,50,50,40,30,30,30,30,30,30,30 SIZE oPainRank:nClientWidth/2,oPainRank:nClientHeight/2 OF oPainRank
...
Static Function IM06A02G()
	...
	_cQuery	:= "SELECT * FROM SZ7010 SZ7															" + CRLF
	_cQuery	+= "WHERE																				" + CRLF
	_cQuery	+= "SZ7.D_E_L_E_T_ = ' '																" + CRLF
	_cQuery	+= "AND SZ7.Z7_MAT BETWEEN '" + MV_PAR01 + "' AND '" + MV_PAR02 + "'					" + CRLF
	_cQuery	+= "AND SZ7.Z7_XDATA BETWEEN '" + DtoS(MV_PAR03) + "' AND '" + DtoS(MV_PAR04) + "'		" + CRLF
//	_cQuery	+= "AND SZ7.Z7_CC BETWEEN '" + MV_PAR05 + "' AND '" + MV_PAR06 + "'					" + CRLF

	MEMOWRITE("IMBLT009.SQL",_cQuery)
	...
	Do While !(cAlias)->(Eof()) 
		...
		nCont++
			...
			aAdd(aCols,{.F.,;
			(cAlias)->Z7_SITU,;	// Filial
			DtoC(StoD((cAlias)->Z7_XDATA)),;
			(cAlias)->Z7_XHORA,;		// Nome Completo
			(cAlias)->Z7_MAT,;		// Data de Nascimento
			(cAlias)->Z7_NOME,;		// Sexo
			DtoC(StoD((cAlias)->Z7_NASC)),;
			(cAlias)->Z7_SEXO,;		// RG
			(cAlias)->Z7_CIC,;	// Estado Civil
			(cAlias)->Z7_RG,;	// Nome Conjuge
			(cAlias)->Z7_ESTCIVI,;	// Igreja Superior
			(cAlias)->Z7_XCONJU,;	// Salário
			(cAlias)->Z7_XIGREJA,;	// Função
			(cAlias)->Z7_SALARIO,;	// Banco Para Deposito de Salário
			(cAlias)->Z7_XTIPO,;	// Tipo de Conta
			(cAlias)->Z7_BCDEPSA,;
			(cAlias)->Z7_CTDEPSA,;	// Numero da Conta
			DtoC(StoD((cAlias)->Z7_ADMISSA)),;	// Data de Admissão
			DtoC(StoD((cAlias)->Z7_DEMISSA)),;	// Data de Demissão
			(cAlias)->Z7_XHRQ,;
			(cAlias)->Z7_CC,;	// Data de Admissão
			cTipo,;
...
Static Function ShowEst()
	...
	Iif(aCols[oEstoque:nAt,2] == 'G',oProc,oImp),;
...
Static Function ShowEnd()
	...
	Iif(aCol1[oEndereco:nAt,2] == 'G',oProc,oImp),;
	...
	aCol1[oEndereco:nAt,17],;
	aCol1[oEndereco:nAt,18]}}
