...
user function 2starenv()//u_starenv()
...
static function printBody()
	...
	local atualI//:=rTrim(posicione("SA1010",1,SRA->(RA_CC),"A1_NOME")) 
   local atualS//:=rTrim(posicione("SA1",1,SRA->(RA_FILIAL+RA_CC),"A1_XDESSUP")) 
	local body  :=''
	local nMat  :=SRA->RA_XTERMO

    body:="Eu, "+rTrim(nome)+", portador (a) da cédula de identidade RG Nº " +rTrim(Rg)+ ", inscrito (a) no CPF/MF sob Nº "+rTrim(cpf)+", sob Matrícula Nº "+allTrim(cvaltochar(nMat))+" declaro que prestava serviços voluntários "+;
           "como "+rTrim(funcao)+" na Igreja Mundial do Poder de Deus de "+rTrim(MV_PAR11)+". Pertencente a sede "+rTrim(MV_PAR12)+". Declaro ainda que nesta data "+dtoc(ddatabase)+" estou me desligando deste Ministério Igreja Mundial do Poder de Deus, em razão do motivo exposto abaixo:"+CRLF
...
static function printInfo()
   ...
   nRow+=50
   oprint:say(nRow,870,"Rua: Carneiro Leão, 439 - Bairro Brás",oFont12B)
   nRow+=50
   oprint:say(nRow,950,"São Paulo-SP CEP:03040-000",oFont12B)
return 
