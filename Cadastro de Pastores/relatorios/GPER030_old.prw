...
±±³Paulo O     ³26/04/17³DRHPAG-578      ³Ajuste para ignorar a opção Quebra por    ³±±
±±³Inzonha 	   ³		³				 ³Funcionário quando impresso direto na porta³±±
±±³            ³        ³                ³Adicionada uma linha antes do cabeçalho    ³±±
±±³            ³        ³				 ³quando não é continuação do recibo anterior³±±
±±³Paulo O     ³04/05/17³DRHPAG-578      ³Ajuste para Adicionar uma linha antes do   ³±±
±±³Inzonha 	   ³		³				 ³Funcionário quando impresso direto na porta³±±
±±³            ³        ³                ³cabeçalho quando  o recibo atual não é     ³±±
±±³            ³        ³				 ³continuação porém o anterior é continuação ³±±
±±³Paulo O.    ³02/06/17³DRHPAG-1640     ³Ajuste no layout do relatório para que     ³±±
±±³Inzonha     ³        ³                ³sejam impressas as três mensagem quando o  ³±±
±±³            ³        ³                ³mesmo não tiver as bases.                  ³±± 
...
nTipRel    := IF( !( lTerminal ), mv_par05 , 3				)	//Tipo de Recibo (Pre-Impressão/Zebrado/EMail/PDF)
...
While SRA->( !Eof() .And. &cInicio <= cFim )
	...
	If nTipRel == 1 .and. !( lTerminal )
		...
		IF !( lTerminal )
			If nTipRel # 3  .And. (aReturn[5] == 3 .Or. aReturn[5] == 2)//--> Só faz teste de impressão quando impressão for direto na porta
				//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
IF !( lTerminal ).And.cPaisLoc<>"PTG" 
	...
	If !Empty(aLog) .And. MsgYesNo(OemToAnsi(STR0208),OemToAnsi(STR0207)) //"Deseja imprimir o LOG das matrículas que não foram enviadas?"#"Atenção"
		fMakeLog(aLog,aTitle,,,"GPER030LOG",OemToAnsi(STR0209),"G","L",,.F.) //"LOG das matrículas que não foram enviadas"
	EndIf
...
Private lContinua := .F.        //identificador se o proximo recibo é continuação do anterior
...
IF !lContinua .AND. lContAnt //adiciona linha quando não for continuação porém o recebo anterior é continuação
	LI ++
Endif
...
ELSE
   oPrint:Say(0370,1015,Alltrim(RA_MUNICIP)+"-"+RA_ESTADO,oArial08)								//Estado e Municipio
ENDIF
...
oPrint:Say(2260,0040,"Recebi da instituição acima identificada, pelo exercicio de ministro religioso a importancia de: " + TRANS((TOTVENC-TOTDESC),cPict1),oArial10N)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³fCabecZArgºAutor  ³Microsiga           º Data ³  02/12/03   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Impressao do Cabecalho - Argentina                         º±±
±±º          ³ Zebrado                                                    º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
*/	@ LI,131 PSAY "|"
	...
	/* CRIA LINHA E IMPRIME MENSAGENS QUANDO IMPRIME BASES = NÃO */	
	...
	ELSE
		LI ++
		@ LI,00  PSAY "| Recebi da instituição acima identificada, pelo exercicio de ministro religioso a importancia de: R$ " + TRANSFORM((TOTVENC-TOTDESC),cPict1) //+ Replicate("_",40)		//"| Recebi o valor acima em ___/___/___ " //"Recebi da instituição acima identificada, pelo exercicio de ministro religioso a importancia de: " + TRANSFORM((TOTVENC-TOTDESC),cPict1),oArial10N)
		@ li,131 PSAY "|"
	ENDIF
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³fRodapeArgºAutor  ³Silvia Taguti       º Data ³  02/12/03   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
IF !( lTerminal )
	...
	If Empty(cEmail)
		If Empty(aTitle)
			Aadd( aTitle, OemToAnsi(STR0209) ) //"LOG das matrículas que não foram enviadas"
			Aadd( aLog,{} )
			Aadd( aLog, { PADR( OemToAnsi(STR0212), FWGETTAMFILIAL) + " - " + PADR( OemToAnsi(STR0001), 10 ) + " - " + PADR( OemToAnsi(STR0003), 45 ) + " - " + OemToAnsi(STR0210) } ) //"Filial"#"Matrícula"#"Nome"#"Ocorrência"
	    EndIf
...
nLi += 10
oPrinter:SayAlign(nLi-8,004,STR0017,oFont4,585,005, /**/, 2, 1 ) //"RECIBO DE PAGAMENTO  "
nLi += 10
...
nLi += 10
oPrinter:Say(nLi,004,Space(3)+Substr(STR0026,3)+cCodFunc+" - "+If( lLpt1, fTAcento(cDescFunc), cDescFunc ),oFont2N)	//"| Funcao    : "
If !Empty(SRA->RA_DEPTO)
...
ORDEMZ ++
oPrinter:Say(nLi,500,STR0025,oFont2N)	//"Ordem : "
oPrinter:Say(nLi,545,StrZero(ORDEMZ,4),oFont2N)
nLi += 10

oPrinter:Say(nLi,004,Space(2) + STR0213 + "  " + Alltrim(Transform(nSalario,cPict2)),oFont2N)	//Salario
