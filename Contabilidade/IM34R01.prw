...
±±³Fun‡…o	 ³ IM34R01  ³ Autor ³ Cicero J. Silva   	³ Data ³ 04.08.06 ³±±
...
*/
User Function IM34R01(cContaIni, cContaFim, dDataIni, dDataFim, cMoeda, cSaldos,cBook, lCusto, cCustoIni, cCustoFim, lItem, cItemIni, cItemFim,lClVl, cClvlIni, cClvlFim,lSaltLin,cMoedaDesc,aSelFil )
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function CTBR400R4(aCtbMoeda,lCusto,lItem,lCLVL,cArqTmp,aSelFil )
	
oReport := ReportDef(aCtbMoeda,lCusto,lItem,lCLVL,@cArqTmp,aSelFil)
oReport:PrintDialog()
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function ReportDef(aCtbMoeda,lCusto,lItem,lCLVL,cArqTmp,aSelFil)
...
Local oSection2
Local oSection3
Local cDesc1		:= "Este programa ir  imprimir o Razao Contabil,"	//"Este programa ir  imprimir o Razao Contabil,"
...
oReport := TReport():New(nomeProg,titulo,cPerg, {|oReport| ReportPrint(oReport,aCtbMoeda,aSetOfBook,cPicture,cDescMoeda,nDecimais,nTamConta,lAnalitico,lCusto,lItem,lCLVL,cArqTmp,aSelFil)},cDesc1+cDesc2+cDesc3)
...
Static Function ReportPrint(oReport,aCtbMoeda,aSetOfBook,cPicture,cDescMoeda,nDecimais,nTamConta,lAnalitico,lCusto,lItem,lCLVL,cArqTmp,aSelFil)
...
If lImpLivro
	...
	Else
		While lImpLivro .And. cArqTmp->(!Eof()) .And. !oReport:Cancel()
			...
			oSection3:Cell("CTRANSP"):SetBlock( { || Iif(nLinha == 7,  "A TRANSPORTAR :", "A TRANSPORTAR :")}) 
...
Local cString		:= "CT2"
Local titulo		:= "Emissao do Razao" 	//"Emissao do Razao Contabil"
Local lAnalitico 	:= .T.
Local lRet			:= .T.
Local lExterno		:= cContaIni <> Nil
Local nTamLinha	:= 220
...
While lImpLivro .And. !cArqTmp->(Eof())
	...
	While cArqTmp->(!Eof()) .And. cArqTmp->CONTA == cContaAnt
		...
		If li > nMaxLin
			...
			@li,aColunas[COL_VLR_TRANSPORTE] - Len("A TRANSPORTAR :") - 1 PSAY "A TRANSPORTAR :"	//"DE TRANSPORTE : "
		...
		While CT1->(!Eof()) .and. CT1->CT1_FILIAL == cFilCT1 .And. CT1->CT1_CONTA <= cContaFim
			...
			While !Eof() .And. CT2->CT2_FILIAL == cFilCT2 .And. CT2->CT2_DEBITO == CT1->CT1_CONTA .and. CT2->CT2_DATA <= dDataFim
				...
				If (CT2->CT2_DC == "1" .Or. CT2->CT2_DC == "3") .And. &(cValid) .And. &(cVldEnt) .And. CT2->CT2_TPSALD == cSaldo
					CT2->(U_CtbGrvRAZ(lJunta,cMoeda,cSaldo,"1",c2Moeda,cAliasCT2,nTipo))
				Endif
				dbSelectArea("CT2")
				dbSkip()
			EndDo
			...
			While !Eof() .And. CT2->CT2_FILIAL == cFilCT2 .And. CT2->CT2_CREDIT == CT1->CT1_CONTA .and. CT2->CT2_DATA <= dDataFim
				...
				If (CT2->CT2_DC == "2" .Or. CT2->CT2_DC == "3") .And. &(cValid) .And. &(cVldEnt) .And. CT2->CT2_TPSALD == cSaldo
					...
					Else
						...
						EndIf			
					EndIf			
					CT2->(U_CtbGrvRAZ(lJunta,cMoeda,cSaldo,"2",c2Moeda,cAliasCT2,nTipo))
				Endif
				dbSelectArea("CT2")
				dbSkip()
			EndDo			
