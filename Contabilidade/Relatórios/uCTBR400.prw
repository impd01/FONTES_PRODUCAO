...
If lOk .And. mv_par32 < 15
		ShowHelpDlg("MINQTDELIN", {"Valor informado inv·lido do","'Num. linhas p/ o raz„o'"},5,{"Favor preencher uma quantidade","mÌnima de 15 linhas"},5)
		lOk := .F.
EndIf
...
If ( lCtbRazBD != nil .and. lCtbRazBD) .and. lProcSld
	...
	If TcSqlExec(cQuery) <> 0
		If !__lBlind
			MsgAlert("CÛdigo e DenominaÁ„o da Conta Cont·bil:"+ cTmpFil+"Erro na exclusao da Procedure:. Excluir manualmente no banco") //"Erro na exclusao da Procedure: "###". Excluir manualmente no banco"
		EndIf
	Endif
	If !Empty(cTmpCTX)
		CtbTmpErase(cTmpCTX)
...
Local oSection2
Local oSection3
Local cDesc1		:= "Este programa ir· imprimir o Raz‰o Contabil,"	//"Este programa ir† imprimir o RazÑo Contabil,"
...
If oReport:GetOrientation() == 1 .or. nTipoRel > 1 // Resumido ou Sintetico
    ...
    MsgAlert("AtenÁ„o, as colunas das entidades Cl Valor, C.Custo e Item Cont·bil  n„o ser„o impressas no modo retrato ou na opÁ„o Resumido.") // "AtenÁ„o, as colunas das entidades Cl Valor, C.Custo e Item Cont·bil  n„o ser„o impressas no modo retrato ou na opÁ„o Resumido.")
...
If lImpLivro
	If cArqTmp->(EoF())
		...
		Aviso("AtenÁ„o","Nao existem dados para os par‚metros especificados.",{"Ok"})
		Return
	Else
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±⁄ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒø±±
±±≥FunáÖo    ≥ CTBR40R3≥ Autor ≥ Pilar S. Albaladejo   ≥ Data ≥ 05.02.01 ≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒ¥±±
...
If Empty(aResult) .Or. aResult[1] = "0"
	MsgAlert(tcsqlerror(), "Erro na geraÁ„o arquivo temporario para relatorio.") //"Erro na geraÁ„o arquivo temporario para relatorio."
	lRet := .F.
Else
	//apaga a procedure apos carregar o temporario
	If TCSPExist( cNomProc )
		If TcSqlExec("DROP PROCEDURE "+cNomProc) <> 0
	 		UserException("Erro na deleÁ„o da Procedure de extraÁ„o dos dados no arquivo temporario." + cNomProc + CRLF + TCSqlError() ) //"Erro na deleÁ„o da Procedure de extraÁ„o dos dados no arquivo temporario."
	   	Else
			lRet := .T.
		EndIf
	Endif
